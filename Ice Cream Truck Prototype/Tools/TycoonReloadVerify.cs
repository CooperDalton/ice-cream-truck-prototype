...
if(g.Parts(0,TycoonPart.Kind.Shelf).First().storage.Locate(TycoonItem.Kind.Bowls)<0)throw new System.Exception("Supplier bowls not restored");
g.NextDay();g.hud.ToggleMap();UnityEngine.ScreenCapture.CaptureScreenshot("Library/CodexPlaytests/TycoonMap.png");
string evidence="Reloaded campaign: level 7, all 16 physical starter rewards retained exactly once, twelve home cooled modules with ten pending placement, installed vanilla full, partial refill retained, purchased bowls on shelf, bicycle position restored.";
