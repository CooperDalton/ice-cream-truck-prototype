...
for(int flavor=0;flavor<2;flavor++)
{
    ...
    p.Gesture(new UnityEngine.Vector2(0,150),.01f);p.Gesture(new UnityEngine.Vector2(0,-150),.01f);p.Gesture(new UnityEngine.Vector2(0,150),.01f);
    ...
    if(g.cash!=(flavor+1)*6)throw new System.Exception("Opening sale payment failed");
    evidence.Add("Sale "+(flavor+1)+": bowl placed, three mouse strokes, scoop deposited, serving collected, cash $"+g.cash+"; tub "+stock+" -> "+tub.contents.amount);
}
if(!g.BuyUpgrade(0,0)||g.cash!=0)throw new System.Exception("First tool purchase failed");
evidence.Add("Two $6 sales funded the $12 improved scooper. Physical tool delivered at home.");
