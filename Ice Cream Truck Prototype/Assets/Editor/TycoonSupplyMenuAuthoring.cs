...
public static class TycoonSupplyMenuAuthoring
{
    ...
    public static void Build()
    {
        ...
        hud.supplyLocks=new GameObject[22];hud.supplyCardContents=new CanvasGroup[22];
        string[] categories={"Ice Cream","Toppings","Supplies","Tools"};
        int[][] products={Enumerable.Range(0,12).ToArray(),Enumerable.Range(12,6).ToArray(),new[]{18,19},new[]{21,20}};
        for(int category=0;category<4;category++)
        {
            ...
            for(int slot=0;slot<products[category].Length;slot++)
            {
                ...
                rect.anchoredPosition=large?new Vector2(-145+slot*290,0):new Vector2(-455+slot%6*182,category==0?85-slot/6*190:0);
                ...
                Sprite icon=product<12?catalog.tubIcons[product]:product<18?catalog.toppingIcons[product-12]:product==18?catalog.bowlPackIcon:product==19?catalog.batterPackIcon:product==20?catalog.improvedIcon:catalog.basicIcon;
                ...
                string name=product<12?TycoonCatalogSO.FlavorNames[product]:product<18?TycoonCatalogSO.ToppingNames[product-12]:product==18?"Bowls":product==19?"Batter":product==20?"High quality scooper":"Basic scooper";
                float price=product<12?TycoonCatalogSO.TubPrices[product]:product<18?TycoonCatalogSO.RefillPrices[product-12]:product==18||product==21?6:12;
